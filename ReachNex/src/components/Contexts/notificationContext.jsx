...
export const NotificationProvider = ({ children }) => {
  ...
  // ✅ Notifications: fetch & socket listener
  useEffect(() => {
    if (!userId) return;
  ...
  // ✅ Unread message count: fetch & socket listener
  useEffect(() => {
    if (!userId) return;
    ...
    // Listen for real-time updates on unread message count
    socket.on("unreadMessageCount", (count) => {
      setUnreadMessageCount(count);
    });
  ...
  // ✅ Connection request count: fetch & socket listener
  useEffect(() => {
    if (!userId) return;
